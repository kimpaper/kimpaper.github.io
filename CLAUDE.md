...
- 작업 브랜치 `claude/blog-structure-design-verkj9` 를 재사용: 시작 시
  `git fetch origin main && git checkout -B claude/blog-structure-design-verkj9 origin/main`
- git identity: `user.email noreply@anthropic.com`, `user.name Claude`
- 흐름: 브랜치 동기화 → 작업 → 로컬 Hugo 빌드 검증 → 커밋/푸시 → PR 생성 → 즉시 머지
  → 라이브 URL 200 확인 (글/이미지/홈 대표글, /en/ 포함. curl 폴링)
- 컨테이너 시계가 틀릴 수 있으니 실제 현재 시각은 `curl -sI https://www.google.com` 의
  Date 헤더로 확인 (글 날짜 미래 방지)
- 연락 이메일은 kimpaper@gmail.com 만 사용 (옛 회사 이메일은 히스토리에서 제거됨 — 재도입 금지)
- 수익화: GA4 `G-ECTTT48V9Y`, AdSense `ca-pub-8048816011496920` (config.yml), `/ads.txt` 배포됨
...
1. **반말 평서체, 짧게.** "~했다", "~한다", "~된다", "~하자".
   문장을 길게 늘이지 말고 뚝뚝 끊는다. 마침표는 생략해도 됨
2. **확신 없으면 얼버무린다.** "~인듯", "~같다", "~할꺼 같다", "~한걸까".
   단정적인 리뷰어 말투 금지
3. **이모지 금지.** 🍲🥂✅ 전부 쓰지 않는다. 옛 글에 이모지가 하나도 없다
4. **마케팅/인플루언서 표현 금지.** "인생 OO", "강력 추천", "JMT", "믿고 먹는" 류 금지.
   단, 사용자가 직접 한 말("최애 메뉴", "아주 좋음" 등)은 그대로 살린다
5. **일상(diary) 글은 감탄사·감정 표현을 적극 넣는다.** 건조하게만 쓰면 밋밋해짐.
   - 감탄사: "크..", "캬..", "와", "어우", "아 이게 여름이지"
   - 감정: "행복하다", "벌써 기대된다", "만족스러웠다", "이 기분 아주 좋다"
   - 느낌표도 자연스럽게 쓴다 ("쫄깃쫄깃!", "아주 좋음!")
   - 단 마케팅투가 아니라 **혼잣말로 터져나오는 진짜 감정**이어야 함.
     기술 글은 예전처럼 건조하게 유지
6. **취소선 혼잣말을 가끔 쓴다.** `~~대부분 그렇지 않을까~~` 같은 자기 농담.
   글 하나에 0~1회. 남발 금지
7. **`..` 말줄임을 가끔 쓴다.** "좋은.. 모듈", "국물 머금은 감자.." 처럼. `...` 아니고 `..`
8. **감성 인용구 마무리 금지.** `> 인용구` 블록 감성 문장, "잘 먹었습니다 🙏" 같은
   클로징 반복 금지. 끝은 혼잣말 한 줄로 ("또 올듯", "밤이 이래서 위험하다")
9. **제목은 간결하게.** "장소/대상 — 메뉴" 정도. 수식어 붙이지 않는다
   (예: "개봉동 대빛식당 — 꽃삼겹", 나쁜 예: "여행 아침은 얼큰하게 —")
